...
export function normalizeLot(lot, index, pricing, paymentState = {}) {
  ...
  const storedStage = getLotStage(lot, paymentState.defaultStage);
  // Lot tracking deliberately begins with the buyer-signed PI. The parent
  // order may still retain its PI-issued history, but every shipment lot is
  // independently actionable from PI Signed onward.
  const currentStage = paymentState.startAtPiSigned && storedStage === 'pi_issued'
    ? 'pi_signed'
    : storedStage;
  ...
  const extraAdvanceAED = Math.max(0, paidAdvanceAED - advanceAED);
  ...
  if (hasRecordedAdvance || advanceReceivedForLot || fullyPaidForLot) {
...
export function createLots({ quantity, lotCount, advancePercent = 20, currentStage = 'pi_signed', ...pricing }) {
  ...
  return distributeLotQuantities(quantity, lotCount).map((lotQuantity, index) => {
    ...
    return normalizeLot({
      ...
      current_stage: currentStage,
    }, index, pricing, { defaultStage: currentStage, startAtPiSigned: true });
  });
}
...
export function applyLotAdvancePayment(lots, totalPaidUSD, pricing) {
  const normalizedLots = normalizeLots(lots, pricing, { defaultStage: 'pi_signed', startAtPiSigned: true });
  ...
  return normalizeLots(normalizedLots.map((lot, index) => ({
    ...
    current_stage: paidByLot[index] > 0 ? 'advance_received' : getLotStage(lot),
  })), pricing, { defaultStage: 'pi_signed', startAtPiSigned: true });
}
...
export function recordLotAdvance(lot, receivedAdvanceUSD, pricing) {
  ...
  const currentStage = paidAdvanceUSD >= totalUSD ? 'got_full_money' : 'advance_received';
